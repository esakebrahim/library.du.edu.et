...
if (!isset($_SESSION['librarian_id'])) {
    die("Access denied. You must log in as a librarian to reject borrow requests.");
}
...
try {
    ...
    // Update borrow_request status to 'borrow_reject'
    $updateRequestQuery = "UPDATE borrow_requests SET status = 'borrow_reject' WHERE id = ?";
    ...
    $notificationMessage = "Dear $student_name, your borrow request for the book '$book_title' has been rejected.";
    ...
    // Log the action of rejecting a borrow request
    $librarian_id = $_SESSION['librarian_id'];
    $action = "Rejected borrow request for the book '$book_title' by user ID: $user_id";
    ...
    header("Location: pending books.php?success=reject");
    exit();
} catch (Exception $e) {
    $conn->rollback(); // Rollback changes on error
    die("Error rejecting borrow request: " . $e->getMessage());
}
